# About SqlLens

## Version Information
**Version:** 1.6.0 
**Release Date:** January 2026  
**Build:** Release  
**Target Framework:** .NET 10

---

## Description
SqlLens is an AI-powered SQL Server Analysis & Visualization that helps developers, DBAs, and consultants understand, document, and maintain complex database systems.

**Key Capabilities:**
- Interactive dependency analysis with drill-down navigation
- Visual dependency charting with Mermaid diagrams
- AI-powered code review and optimization suggestions
- Cross-database and multi-server search
- Database comparison tools across environments

---

## Community & Support

### GitHub Resources
- **Community Discussions:** https://github.com/jcboyer/sqllens-docs-docs/discussions
- **Issue Tracking:** https://github.com/jcboyer/sqllens-docs-docs/issues
- **Source Code:** https://github.com/jcboyer/sqllens-docs

### Getting Help
- Quick Start Guide (Help → Quick Start)
- User Guide (Help → User Guide)
- Reference Guide (Help → Reference Guide)

---

## Credits

### Development Team
**Lead Developer:** Jerome Boyer  
**Project Type:** Proprietary Software (Beta)  
**Started:** October 2025

### Technologies
- .NET 10 / WPF
- Grok AI (x.ai)
- Microsoft Azure
- Mermaid.js Charting
- Markdig Markdown

---

## License

### License Information

**Full License:** https://github.com/jcboyer/sqllens-docs/blob/main/LICENSE.md

**Copyright © 2024-2026 Jerome Charles Boyer**

See the LICENSE file for complete terms and conditions.

---

## Beta Program
**Thank you for participating in the SqlLens beta!**

**Current Status:** Beta Testing  
**Beta Started:** October 2025  
**Free Access Period:** Until March 2026  
**Expected Release:** Q1 2026

### Provide Feedback
1. Report Bugs - Help → Community & Support → Report Issue
2. Request Features - Help → Community & Support → Community Discussions
3. Share Success Stories - Post in GitHub Discussions

### After Beta
Following the beta period, SqlLens will transition to a commercial licensing model to support ongoing development and AI service costs.

---

**Thank you for using SqlLens!** 🎉

*Last Updated: January 2026*
