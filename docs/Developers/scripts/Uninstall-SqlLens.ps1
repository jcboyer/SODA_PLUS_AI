...
<#
.SYNOPSIS
 Uninstall SqlLens from Windows
.DESCRIPTION
    Removes all versions of SqlLens MSIX packages
.EXAMPLE
    .\Uninstall-SODA.ps1
#>
...
Write-Host "  🗑️  SqlLens Uninstaller" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""

# Find all SqlLens packages
$packages = Get-AppxPackage | Where-Object { 
    $_.Name -like "*SqlLensai*" -or 
    $_.Name -like "*SODA*" -or
    $_.Publisher -like "*jerom*"
}

if (-not $packages) {
    Write-Host "✅ SqlLens is not installed" -ForegroundColor Green
    Write-Host ""
    exit 0
}
...
if ($uninstalledCount -gt 0) {
    Write-Host "✅ SqlLens has been removed from your system" -ForegroundColor Green
    Write-Host ""
}
...
if ($cleanResponse -eq 'y') {
    ...
    $cleanupPaths = @(
     "$env:LOCALAPPDATA\Packages\*SqlLensai*",
